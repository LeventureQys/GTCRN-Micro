...
def mk_mixture(s1, s2, s1_ref, snr, eps=1e-8):
    ...
    norm_sig2 = (
        s2 * np.math.sqrt(np.sum(s1**2) + eps) / np.math.sqrt(np.sum(s2**2) + eps)
    )
    alpha = 10 ** (-snr * 1.5 / 20)
    ...
    M = (
        max(np.max(abs(mix)), np.max(abs(norm_sig1)), np.max(abs(alpha * norm_sig2)))
        + eps
    )
    if M > 1.0:
        mix = mix / M
...
if __name__ == "__main__":
    ...
    save_root = "./gtcrn_micro/data"

    data_root = "./gtcrn_micro/data/"
    clean_csv = os.path.join(data_root, f"dns3_csvs/{flag}_clean_dir.csv")
    noise_csv = os.path.join(data_root, f"dns3_csvs/{flag}_noise_dir.csv")
    rir_csv = os.path.join(data_root, f"dns3_csvs/{flag}_rir_dir.csv")
    ...
    info = pd.DataFrame(
        [str(idx + 1).zfill(nfill) + ".wav" for idx in range(num_tot)],
        columns=["file_name"],
    )
    ...
    for idx in tqdm(range(num_tot)):
        ...
        clean = sf.read(
            clean_list[idx], dtype="float32", start=start_s, stop=start_s + wav_len * fs
        )[0]
        noise = sf.read(
            noise_list[idx], dtype="float32", start=start_n, stop=start_n + wav_len * fs
        )[0]
        ...
        rir = rir[max_index:]
        rir_e = rir[: min(int(100 * 16000 / 1000), len(rir))]
        ...
        mixture, target = mk_mixture(
            rev_clean, noise, drb_clean, snr_list[idx], eps=1e-8
        )

        sf.write(
            os.path.join(
                save_root, f"{flag}_noisy", str(idx + 1).zfill(nfill) + ".wav"
            ),
            mixture,
            fs,
        )
        sf.write(
            os.path.join(
                save_root, f"{flag}_clean", str(idx + 1).zfill(nfill) + ".wav"
            ),
            target,
            fs,
        )
